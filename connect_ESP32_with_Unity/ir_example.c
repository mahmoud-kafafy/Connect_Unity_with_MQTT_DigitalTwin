...
void ir_sensor_task(void *pvParameters) {
    int last_state = 1; // assume sensor idle HIGH at start

    while (1) {
        int sensor_state = gpio_get_level(IR_SENSOR_GPIO);

        if (sensor_state == 0 && last_state == 1) {
            // Object detected (falling edge)
            if (client != NULL && mqtt_connected) {
                esp_mqtt_client_publish(client, "unity/conveyor/control", "fwd", 0, 1, 0);
                printf("IR detected → Sent 'fwd' to unity\n");
            } else {
                printf("MQTT not connected, cannot publish IR event\n");
            }
        }
...
void app_main() {
    ...
    .pull_up_en = 1,    // Enable internal pull-up if needed
    .pull_down_en = 0,
    ...
    xTaskCreate(&ir_sensor_task, "ir_sensor", 4096, NULL, 5, NULL);
